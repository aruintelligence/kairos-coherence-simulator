...
A lightweight, reproducible single-file simulator that models inner coherence
through stochastic dynamics, an ethical "guardian veto", and gentle
Dzogchen-inspired reflections (Trekchö & Tögal).

This is a poetic / contemplative tool for reflection and self-observation —
not literal enlightenment software or a deep meditative simulator.

Features
--------
- Fully reproducible with isolated RNG
- Defensive input validation
- Lazy Plotly import + optional HTML export
- Rich summary and stable JSON logging
- Clean, extensible CLI with multiple modes
- Resettable simulator state for repeatable reruns
- JSON-safe serialization with proper bool handling
...
class SimulationConfig:
    """Configuration for the coherence simulation with full validation."""
    ...
    def validate(self) -> None:
        ...
        if self.inward_std < 0.0:
            raise ValueError("inward_std must be >= 0.0")
        if self.outward_std < 0.0:
            raise ValueError("outward_std must be >= 0.0")
...
class KairosEchoPrototype:
    ...
    def reset(self) -> None:
        """Reset simulator state and history while preserving configuration and re-seeding RNG."""
    ...
    def love_speed_collapse(self) -> float:
        """Apply bounded tanh collapse for rapid convergence."""
    ...
    def guardian_veto(self, proposed_score: float) -> bool:
        """Veto ambitious outward moves when coherence is below threshold."""
        return (
    ...
    def compute_togal_stage(coherence: float) -> float:
        """Map coherence onto a [0, 4] proxy for the Four Visions."""
    ...
    def step(self, action_type: InputActionType = "inward") -> tuple[float, float, bool, float]:
        ...
        if vetoed:
            self.coherence = max(
                self.config.veto_floor,
                self.coherence - self.config.veto_penalty,
            )
    ...
    def summary(self) -> RunSummary:
        ...
        mu_mean = self.history["mu_mean"]
        actions = self.history["action"]
        vetoes = self.history["vetoed"]
        togal = self.history["togal_stage"]

        return RunSummary(
            timestamp_utc=datetime.now(timezone.utc).isoformat(),
            steps=len(coherence),
            final_coherence=float(coherence[-1]),
            min_coherence=float(min(coherence)),
            max_coherence=float(max(coherence)),
            mean_coherence=float(np.mean(coherence)),
            final_mu_mean=float(mu_mean[-1]),
            mean_mu=float(np.mean(mu_mean)),
            veto_count=int(sum(vetoes)),
            inward_count=int(sum(1 for a in actions if a == "inward")),
            outward_count=int(sum(1 for a in actions if a == "outward")),
            vetoed_action_count=int(sum(1 for a in actions if a == "vetoed")),
            final_togal_stage=float(togal[-1]),
...
def generate_togal_reflection(coherence: float, togal_stage: float) -> str:
    ...
    prompts = {
        "The Absolute Nature Becoming Manifest":
            "Rest in the naked rigpa that is already here.",
        "The Experience of Increasing Appearances":
            "Let visions arise and self-liberate without grasping.",
        "Awareness Reaching Its Greatest Magnitude":
            "The clear light remembers itself through you. Abide in luminosity.",
        "The Exhaustion of Phenomena in Dharmata":
            "All appearances dissolve into the ground — exhaustion into reality itself.",
    }
    return (
...
def plot_togal_visualizer(history: Dict[str, List[Any]]) -> Any:
    ...
    x_values = list(range(len(history["coherence"])))
    fig = make_subplots(
        rows=2,
        cols=1,
        subplot_titles=(
            "Coherence & Tögal Stage",
            "μ Field Mean (Love Speed Collapse)",
        ),
...
def serialize_history(history: Dict[str, List[Any]]) -> Dict[str, List[Any]]:
    """Convert history to JSON-safe types without corrupting bools."""
...
def export_symbiosis_log(
    ...
    action_summary = {
        "inward": int(sum(1 for action in actions if action == "inward")),
        "outward": int(sum(1 for action in actions if action == "outward")),
        "vetoed": int(sum(1 for action in actions if action == "vetoed")),
    }
...
def print_summary(summary: RunSummary) -> None:
    ...
    print(f"Inward actions:      {summary.inward_count} ({summary.inward_count / total * 100:.1f}%)")
    print(f"Outward actions:     {summary.outward_count} ({summary.outward_count / total * 100:.1f}%)")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.75,
        help="Probability of inward action each step",
    )
    parser.add_argument(
        "--save-html",
        type=str,
        default="",
        help="Save dashboard to this HTML file (dashboard mode only)",
    )
    parser.add_argument(
        "--log-file",
        type=str,
...
def main() -> int:
    try:
        ...
        if args.mode == "dashboard":
            ...
            if args.save_html:
                output_path = Path(args.save_html).expanduser().resolve()
